...
class ProposalCreator():
    def __init__(self,mode,nms_iou=0.7,
        ...
        self.mode = mode # 预测还是训练
        self.nms_iou = nms_iou
        #   训练用到的建议框数量
        self.n_train_pre_nms = n_test_pre_nms
...
class RegionProposalNetwork(nn.Module):
 ...
 #   先进行一个3x3的卷积，可理解为特征整合
        ...
        #   分类预测先验框内部是否包含物体
        self.score = nn.Conv2d(mid_channels, n_anchor * 2, 1, 1, 0) # 有和没有i的区别
        #   回归预测对先验框进行调整
        self.loc = nn.Conv2d(mid_channels,n_anchor*4,1,1,0)
        #   特征点间距步长
        self.feat_stride = feat_stride
        #   用于对建议框解码并进行非极大抑制
        self.propasal_layer = ProposalCreator(mode)
        #   对FPN的网络部分进行权值初始化
